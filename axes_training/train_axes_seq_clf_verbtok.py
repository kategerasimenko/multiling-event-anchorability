...
def main(
    ...
    include_brackets: bool = typer.Option(False, help='Whether to include brackets around the verb in the input.'),
    ...
    model_name = f'axes_seq_verbtok_{base_model}_lr{learning_rate}_e{max_epochs}_bs{batch_size}_sw{do_sample_weights}_br{include_brackets}'
    ...
    axes_df, ds = process_english_data(
        DATA_DIR,
        task='seq_clf_verbtok',
        tokenizer=tokenizer,
        processing_params={'include_brackets': include_brackets},
        seed=SEED
    )
    ...
    neptune_callback.run['include_brackets'] = include_brackets
